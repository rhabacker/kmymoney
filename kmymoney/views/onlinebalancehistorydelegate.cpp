...
QColor OnlineBalanceHistoryDelegate::m_erroneousColor = QColor(Qt::red);
QColor OnlineBalanceHistoryDelegate::m_importedColor = QColor(Qt::yellow);
QColor OnlineBalanceHistoryDelegate::m_separatorColor = QColor(0xff, 0xf2, 0x9b);

class OnlineBalanceHistoryDelegate::Private
{
public:
    Private()
        : m_editor(nullptr)
...
OnlineBalanceHistoryDelegate::OnlineBalanceHistoryDelegate(LedgerView* parent)
...
void OnlineBalanceHistoryDelegate::setErroneousColor(const QColor& color)
{
    m_erroneousColor = color;
}

void OnlineBalanceHistoryDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    const auto onlineBalanceDate = index.data(eMyMoney::Model::AccountOnlineBalanceHistoryDateRole).toDate();
    if (onlineBalanceDate.isValid()) {
        onlineBalanceDateTxt = MyMoneyUtils::formatDate(onlineBalanceDate);
    }
    const auto onlineBalanceValue = index.data(eMyMoney::Model::AccountOnlineBalanceHistoryValueRole).value<MyMoneyMoney>();
    ...
    case JournalModel::Column::Detail:
        ...
        painter->drawText(opt.rect, Qt::AlignCenter, i18nc("Label in ledger for online balance row", "Online Balance (History)"));
        break;
...
QSize OnlineBalanceHistoryDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    QStyleOptionViewItem opt = option;
    initStyleOption(&opt, index);
    QStyle* style = opt.widget ? opt.widget->style() : QApplication::style();
 ...
 */
bool OnlineBalanceHistoryDelegate::eventFilter(QObject* o, QEvent* event)
{
