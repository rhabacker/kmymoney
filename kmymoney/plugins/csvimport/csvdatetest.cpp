...
void CsvDateTest::testDateConvert()
{
  ...
  QVERIFY(m_convert->convertDate("13/09/81") ==
          MyMoneyDate::fromString("13/09/1981", format));  //a = "13/09/81"

  QVERIFY(m_convert->convertDate("13/09/01") ==
          MyMoneyDate::fromString("13/09/2001", format));  //b = "13/09/01"

  QVERIFY(m_convert->convertDate("13-09-81") ==
          MyMoneyDate::fromString("13/09/1981", format));  //c = "13-09-81"

  QVERIFY(m_convert->convertDate("13-09-01") ==
          MyMoneyDate::fromString("13/09/2001", format));  //d = "13-09-01"

  QVERIFY(m_convert->convertDate(QString("25-" + MyMoneyDate::longMonthName(12) + "-2000")) ==
          MyMoneyDate::fromString("25/12/2000", format));  //e = "25-December-2000"
  ...
  //MyMoneyDate::shortMonthName(11) + "-1999")) ==
  //MyMoneyDate::fromString("5/11/1999", format));//f = "5-Nov-1999"

  QVERIFY(m_convert->convertDate("13.09.81") ==
          MyMoneyDate::fromString("13/09/1981", format));  //g = "13.09.81"

  QVERIFY(m_convert->convertDate("32/01/2000") ==
          MyMoneyDate());//                             h ="32/01/2000" invalid day

  QVERIFY(m_convert->convertDate(QLatin1String("13-rubbishmonth-2000")) ==
          MyMoneyDate());//        i = "13-rubbishmonth-2000" invalid month

  QVERIFY(m_convert->convertDate("01/13/2000") ==
          MyMoneyDate());//                  j = "01/13/2000"  invalid month

  QVERIFY(m_convert->convertDate("01/12/200") ==
          MyMoneyDate());//                   k = "01/12/200"  invalid year

  QVERIFY(m_convert->convertDate("") ==
          MyMoneyDate());//                            l = ""   empty date

  format = "ddMMyyyy";
  QVERIFY(m_convert->convertDate("31-1-2010") ==
          MyMoneyDate::fromString("31012010", format));  //m = "31-1-2010" single digit month

// Now with no separators

  QVERIFY(m_convert->convertDate("13091981") ==
          MyMoneyDate(1981, 9, 13));
}
...
void CsvDateTest::testDateConvertFormats()
{
  ...
  QVERIFY(m_convert->convertDate(aDate) == MyMoneyDate::fromString("2001/11/30", format));

  m_convert->setDateFormatIndex(1);  //           US date format

  QVERIFY(m_convert->convertDate(aDate) == MyMoneyDate());
...
// Now with no separators
  ...
  QVERIFY(m_convert->convertDate(aDate) == MyMoneyDate(2001, 11, 30));
  ...
  QVERIFY(m_convert->convertDate(aDate) == MyMoneyDate::fromString("11/30/2001", format));
