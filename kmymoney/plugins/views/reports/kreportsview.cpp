...
void KReportsView::slotExportView()
{
    ...
    if (auto tab = dynamic_cast<KReportTab*>(d->ui.m_reportTabWidget->currentWidget())) {
        ...
        dialog->setMimeTypeFilters({QStringLiteral("text/csv"), QStringLiteral("text/html"), QStringLiteral("application/pdf")});
...
void KReportsView::slotListContextMenu(const QPoint & p)
{
    ...
    if (tocItems.count() == 1) {
        ...
        contextmenu->addAction(i18nc("To export a report", "&Export"), this, SLOT(slotExportFromList()));
    }
...
void KReportsView::slotExportFromList()
{
    ...
    Q_FOREACH (auto item, items) {
        ...
        int index = 0;
        do {
            fi.setFile(rootDir + QDir::separator() + name + suffix + QLatin1String(".pdf"));
            suffix = QString(QLatin1String("-%1")).arg(++index);
        } while (fi.exists());
        tab->saveAs(fi.absoluteFilePath(), "application/pdf");
    }
}
