...
void ListTable::render(QString& result, QString& csv) const
{
    ...
    for (const auto& cellType : std::as_const(columns)) {
        result.append(QString::fromLatin1("<th title=\"%1\">%2</th>").arg(tableToolTip(cellType), tableShortHeader(cellType)));
...
QString ListTable::tableShortHeader(const cellTypeE cellType)
{
    switch (cellType) {
    case ctAnnualizedReturn:
