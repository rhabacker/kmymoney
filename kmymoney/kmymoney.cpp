...
#define STRING_WRAPPER(a) a
#else
#define STRING_WRAPPER(a) QString::fromLatin1(a)
#endif
...
public:
    ...
    bool askAboutSaving()
    {
        const auto isFileNotSaved = q->actionCollection()->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::Save)))->isEnabled();
    ...
    void updateActions(const SelectedObjects& selections)
    {
        ...
        aC->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::SaveAs)))->setEnabled(canFileSaveAs());
        aC->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::Close)))->setEnabled(m_storageInfo.isOpened);
    ...
    void fileAction(eKMyMoney::FileAction action)
    {
        ...
        case eKMyMoney::FileAction::Opened:
            q->actionCollection()->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::Save)))->setEnabled(false);
            updateAccountNames();
            updateCurrencyNames();
            selectBaseCurrency();
            ...
            q->actionCollection()->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::Save)))->setEnabled(dirty());
        ...
        case eKMyMoney::FileAction::Changed:
            q->disconnect(MyMoneyFile::instance(), &MyMoneyFile::dataChanged, q, &KMyMoneyApp::slotDataChanged);
            q->actionCollection()->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::Save)))->setEnabled(true && !m_storageInfo.url.isEmpty());
...
QHash<Action, QAction *> KMyMoneyApp::initActions()
{
    ...
    */
    ...
    auto aboutApp = aC->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::AboutApp)));
    aboutApp->disconnect();
...
void KMyMoneyApp::slotUpdateConfiguration(const QString &dialogName)
{
    if(dialogName.compare(QLatin1String("Plugins")) == 0) {
        ...
        actionCollection()->action(STRING_WRAPPER(KStandardAction::name(KStandardAction::SaveAs)))->setEnabled(d->canFileSaveAs());
        onlineJobAdministration::instance()->updateActions();
        onlineJobAdministration::instance()->setOnlinePlugins(pPlugins.extended);
        d->m_myMoneyView->setOnlinePlugins(&pPlugins.online);
        d->updateActions(d->m_selections);
