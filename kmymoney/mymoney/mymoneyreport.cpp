...
QDebug operator<<(QDebug dbg, const MyMoneyReport &a)
{
  ...
  return d
    << L("accountGroups") << list
    << L("budget") << a.budget()
    << L("chartLineWidth") << a.chartLineWidth()
    << L("chartType") << a.chartType()
    << L("columnPitch") << a.columnPitch()
    << L("columnType") << a.columnType()
    << L("comment") << a.comment()
    << L("currentDateColumn") << a.currentDateColumn()
    << L("detailLevel") << a.detailLevel()
    << L("name") << a.name()
    << L("group") << a.group()
    << L("hasBudget") << a.hasBudget()
    << L("id") << a.id()
    << L("isConvertCurrency") << a.isConvertCurrency()
    << L("isFavorite") << a.isFavorite()
    << L("isInvestmentsOnly") << a.isInvestmentsOnly()
    << L("isChartDataLabels") << a.isChartDataLabels()
    << L("isChartGridLines") << a.isChartGridLines()
    << L("isChartByDefault") << a.isChartByDefault()
    << L("isColumnsAreDays") << a.isColumnsAreDays()
    << L("isIncludingAveragePrice") << a.isIncludingAveragePrice()
    << L("isIncludingBudgetActuals") << a.isIncludingBudgetActuals()
    << L("isIncludingForecast") << a.isIncludingForecast()
    << L("isIncludingMovingAverage") << a.isIncludingMovingAverage()
    << L("isIncludingPrice") << a.isIncludingPrice()
    << L("isIncludingSchedules") << a.isIncludingSchedules()
    << L("isIncludingTransfers") << a.isIncludingTransfers()
    << L("isIncludingUnusedAccounts") << a.isIncludingUnusedAccounts()
    << L("isLoansOnly") << a.isLoansOnly()
    << L("isMixedTime") << a.isMixedTime()
    << L("isRunningSum") << a.isRunningSum()
    << L("isSkippingZero") << a.isSkippingZero()
    << L("isShowingColumnTotals") << a.isShowingColumnTotals()
    << L("isShowingRowTotals") << a.isShowingRowTotals()
    << L("isTax") << a.isTax()
    << L("isUserDefined") << a.isUserDefined()
    << L("queryColumns") << a.queryColumns()
    << L("movingAverageDays") << a.movingAverageDays()
    << L("rowType") << MyMoneyReport::toString(a.rowType())
    << L("reportType") << MyMoneyReport::toString(a.reportType())
