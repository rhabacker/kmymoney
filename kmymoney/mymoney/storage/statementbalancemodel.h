...
#ifndef STATEMENTBALANCEMODEL_H
#define STATEMENTBALANCEMODEL_H
...
class /* no export here on purpose */ StatementBalanceEntry
{
public:
    explicit StatementBalanceEntry()
    {
    }
    explicit StatementBalanceEntry(const QString& id, const StatementBalanceEntry& other)
        : m_id(id)
    ...
    StatementBalanceEntry(const QString& id,
...
/**
 */
class KMM_MYMONEY_EXPORT StatementBalanceModel : public MyMoneyModel<StatementBalanceEntry>
{
    Q_OBJECT

public:
    explicit StatementBalanceModel(QObject* parent = nullptr, QUndoStack* undoStack = nullptr);
    ~StatementBalanceModel();
...
private Q_SLOTS:
     ...
     */
    void load(const QMap<QString, StatementBalanceEntry>& list);
    void doLoad();
