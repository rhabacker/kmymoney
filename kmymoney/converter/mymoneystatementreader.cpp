                                                                 ...
                                                                 const TransactionMatchFinder::MatchResult& matchResult)
{
    ...
    case TransactionMatchFinder::MatchDuplicate:
        if (matchedTransaction.splits()[0].memo().isEmpty() && !importedTransaction.splits()[0].memo().isEmpty()) {
            ...
            for (it_m = matchedTransaction.splits().begin(), it_i = importedTransaction.splits().begin(); it_m != matchedTransaction.splits().end();
                 ++it_m, ++it_i) {
            ...
            if (!matchedTransaction.splits()[0].memo().isEmpty() && !importedTransaction.splits()[0].memo().isEmpty()) {
                qDebug("id:'%s' m:'%s' i:'%s'\n",
                       qPrintable(matchedTransaction.id()),
                       qPrintable(matchedTransaction.splits()[0].memo()),
                       qPrintable(importedTransaction.splits()[0].memo()));
            }
